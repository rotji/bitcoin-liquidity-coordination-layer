...
describe('AdvancedDatabaseAdapter - end-to-end workflow', () => {
  ...
  it('should handle a multi-step transaction workflow', async () => {
    ...
    const ops = [
      { type: 'update', id: 'asset-1', value: { symbol: 'BTC', liquidity: 15 } },
      { type: 'update', id: 'asset-2', value: { symbol: 'ETH', liquidity: 25 } },
    ] as Array<{ type: 'update' | 'create' | 'delete'; id: string; value?: any }>;
    const result = await adapter.transaction(ops);
    expect(result.success).toBe(true);
    expect(await adapter.read('asset-1')).toEqual({ symbol: 'BTC', liquidity: 15 });
    expect(await adapter.read('asset-2')).toEqual({ symbol: 'ETH', liquidity: 25 });
  });
});
