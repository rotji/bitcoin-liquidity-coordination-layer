...
async function apiRouter(
  ...
  if (route === "/user/create") {
    ...
    return {
      status: 201,
      data: { id: user.id, reputation: user.reputationScore },
    };
  }
  if (route === "/asset/get") {
    if (!req.id) return { status: 400, error: "Missing asset id" };
    if (!assetRegistry)
      return { status: 500, error: "Asset registry unavailable" };
    const asset = await assetRegistry.getAssetById(req.id);
    return asset
      ? { status: 200, data: asset }
      : { status: 404, error: "Asset not found" };
  }
