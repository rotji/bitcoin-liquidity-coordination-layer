...
describe("CommitLiquiditySignal", () => {
  it("should commit a liquidity signal using the port", async () => {
    const mockCommit = jest.fn((_: any) => Promise.resolve()) as (
      snapshot: LiquiditySnapshot,
    ) => Promise<void>;
    const mockGetLatestSignal = jest.fn((_: string) =>
      Promise.resolve(null),
    ) as (poolId: string) => Promise<LiquiditySnapshot | null>;
    const mockGetSignalByHeight = jest.fn((_: string, __: number) =>
      Promise.resolve(null),
    ...
    const commitLiquiditySignal = new CommitLiquiditySignal(
      mockLiquiditySignal,
    );
