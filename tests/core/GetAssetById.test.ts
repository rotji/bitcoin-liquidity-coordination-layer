...
describe("GetAssetById", () => {
  it("should fetch asset by id using the port", async () => {
    ...
    const mockGetAssetById = jest.fn((id: string) =>
      Promise.resolve(id === "BTC" ? mockAsset : null),
    ) as (id: string) => Promise<Asset | null>;
    const mockAssetRegistry: AssetRegistry = {
      getAssets: jest.fn(() => Promise.resolve([mockAsset])) as () => Promise<
        Asset[]
      >,
  ...
  it("should return null if asset not found", async () => {
    const mockGetAssetById = jest.fn(() => Promise.resolve(null)) as (
      id: string,
    ) => Promise<Asset | null>;
    const mockAssetRegistry: AssetRegistry = {
