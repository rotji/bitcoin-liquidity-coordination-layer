...
describe("GetLiquiditySignalByHeight", () => {
  it("should fetch liquidity signal by poolId and height", async () => {
    const mockSnapshot: LiquiditySnapshot = new LiquiditySnapshot(
      "pool-1",
      123,
      1.5,
      300,
    );
    const mockGetSignalByHeight = jest.fn((poolId: string, height: number) =>
      Promise.resolve(
        poolId === "pool-1" && height === 123 ? mockSnapshot : null,
      ),
    ) as (poolId: string, height: number) => Promise<LiquiditySnapshot | null>;
    const mockCommit = jest.fn((_: LiquiditySnapshot) => Promise.resolve()) as (
      snapshot: LiquiditySnapshot,
    ) => Promise<void>;
    const mockGetLatestSignal = jest.fn((_: string) =>
      Promise.resolve(null),
    ) as (poolId: string) => Promise<LiquiditySnapshot | null>;
    ...
    const getLiquiditySignalByHeight = new GetLiquiditySignalByHeight(
      mockLiquiditySignal,
    );
  ...
  it("should return null if no signal found", async () => {
    const mockGetSignalByHeight = jest.fn(() => Promise.resolve(null)) as (
      poolId: string,
      height: number,
