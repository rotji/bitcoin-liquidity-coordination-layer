...
describe("GetProtocolById", () => {
  it("should fetch protocol by id using the port", async () => {
    ...
    const mockGetProtocolById = jest.fn((id: string) =>
      Promise.resolve(id === "proto-1" ? mockProtocol : null),
    ) as (id: string) => Promise<Protocol | null>;
    const mockProtocolRegistry: ProtocolRegistry = {
      getProtocols: jest.fn(() =>
        Promise.resolve([mockProtocol]),
      ) as () => Promise<Protocol[]>,
  ...
  it("should return null if protocol not found", async () => {
    const mockGetProtocolById = jest.fn(() => Promise.resolve(null)) as (
      id: string,
    ) => Promise<Protocol | null>;
    const mockProtocolRegistry: ProtocolRegistry = {
      getProtocols: jest.fn(() => Promise.resolve([])) as () => Promise<
        Protocol[]
      >,
      getProtocolById: mockGetProtocolById,
    };
