...
describe('GetProtocols', () => {
  it('should fetch protocols using the port', async () => {
    ...
    const mockGetProtocols = jest.fn(() => Promise.resolve(mockProtocols)) as () => Promise<Protocol[]>;
    const mockProtocolRegistry: ProtocolRegistry = {
      getProtocols: mockGetProtocols,
      getProtocolById: jest.fn((_: string) => Promise.resolve(null)) as (id: string) => Promise<Protocol | null>,
    };
