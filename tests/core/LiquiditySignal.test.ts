...
describe('LiquiditySignal interface', () => {
  it('should allow mocking all required methods', async () => {
    ...
    const mockLiquiditySignal: LiquiditySignal = {
      commitLiquiditySignal: jest.fn((_: LiquiditySnapshot) => Promise.resolve()),
      getLatestSignal: jest.fn((poolId: string) => Promise.resolve(poolId === 'pool-1' ? mockSnapshot : null)),
      getSignalByHeight: jest.fn((poolId: string, height: number) => Promise.resolve((poolId === 'pool-1' && height === 123) ? mockSnapshot : null)),
    };
