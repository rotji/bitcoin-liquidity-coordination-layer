...
describe("Integration: create user → update reputation → publish intent", () => {
  it("should create a user, update reputation, and publish routing intent", async () => {
    ...
    const mockUpdateScore = jest.fn((_: string, __: number) =>
      Promise.resolve(),
    ) as (userId: string, score: number) => Promise<void>;
    const mockGetScore = jest.fn((_: string) => Promise.resolve(42)) as (
      userId: string,
    ) => Promise<number>;
    ...
    const mockPublishIntent = jest.fn((_: object) => Promise.resolve()) as (
      intent: object,
    ) => Promise<void>;
    const mockGetIntents = jest.fn(() =>
      Promise.resolve([{ from: "BTC", to: "ETH", amount: 10 }]),
    ) as () => Promise<object[]>;
